...
const App = () => {
  ...
  const items = [
    ...
    { id: "portal",           l: "App paciente",     v: () => { setView("portal"); } },
  ];
  ...
  const isPortalMobile = isMobile && view === "portal";
  ...
  return (
    <div className="flex" style={{ minHeight: "100vh", background: t.bg, color: t.text }}>
      ...
      <div className="flex-1 min-w-0">
        ...
        {isMobile && !isPortalMobile && <MobileBranchStrip theme={theme} branchId={branchId} setBranchId={setBranchId}/>}
        <div style={{ paddingBottom: isPortalMobile ? 0 : (isMobile ? 80 : 70) }}>
          {renderView()}
        </div>
      </div>
      {!isPortalMobile && <QuickNav/>}
    </div>
  );
};
