...
const NAV = [
  ...
  { id: "portal",    label: "App paciente",  icon: IconUser, divider: true },
];
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <div className="leading-none min-w-0">
        <div className="text-[15px] font-semibold tracking-tightish" style={{ color: t.text }}>Reha<span style={{ color: t.accent }}>.</span>mx</div>
        ...
        {NAV.map((n) => {
          ...
          return (
            <React.Fragment key={n.id}>
              {n.divider && <div className="my-2 mx-2 border-t" style={{ borderColor: t.border }}/>}
              {n.divider && <div className="text-[10px] font-mono uppercase tracking-wider px-2.5 pt-1 pb-1" style={{ color: t.subtle }}>Cara del paciente</div>}
              <button onClick={() => { setView(n.id); onNavigate && onNavigate(); }}
    ...
    <header className="sticky top-0 z-20 flex items-center gap-2 px-3 sm:px-5 py-2.5"
      ...
      {!isMobile && (
        <div className="flex items-center gap-2 text-[12px] font-mono no-wrap" style={{ color: t.subtle }}>
          ...
          <span style={{ color: t.muted }}>Reha</span>
          <IconChevronRight size={11}/>
