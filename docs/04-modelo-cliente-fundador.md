...
(empresa nueva, propiedad de Antonio)     (clínica de readaptación SLP)
       ...
       │  ─────────────────────────────────────►  │
       │                                          │
       │  Acceso ilimitado y vitalicio + soporte  │
       │  prioritario + influencia en roadmap     │
       │  ─────────────────────────────────────►  │
       │                                          │
       │  ◄─ Caso de éxito + testimonios + ────  │
       │     feedback constante + costos          │
       │     operativos pass-through              │
       │                                          │
```

**Sin equity. Sin revenue share. Sin transferencia de código. Sin pago de desarrollo.**

---

## 💰 Estructura económica

### Lo que paga MoveWell
...
- CFDIs Facturama (~$600 MXN, ajustable a uso real)
- Resend/email transaccional (~$200 MXN)
...
- ❌ Costo de desarrollo del SaaS ($510k MXN originales) — se cancela.
- ❌ Licencias de software propietario.
- ❌ Honorarios de KonectAI por mantenimiento.
- ❌ Cargos por nuevas features liberadas en el roadmap general.
...
- Infraestructura base (la parte fija de Vercel/Supabase no atribuible al tenant).
- Soporte (porque MoveWell es cliente fundador, no porque haya un SLA pagado).

---

## 🎁 Beneficios vitalicios para MoveWell
...
3. **Feedback de calidad** porque MoveWell tiene skin in the game (su operación depende del SaaS).
4. **Costo de cliente fundador es bajo** — la infra ya está corriendo de todos modos para construir el producto.
5. **Sin obligación de entregar código** = preserva el activo principal (la propiedad intelectual del SaaS).
6. **Modelo replicable** — el cliente #2 paga cuota normal, el #3 también, y MoveWell sigue gratis (es justo: ellos llegaron primero).
...
**Domicilio fiscal:** San Luis Potosí, S.L.P.

**Régimen fiscal:** Persona moral, ISR + IVA. Régimen general.
...
## 📝 Plantilla — Contrato de Cliente Fundador
...
Vitalicia mientras MoveWell:
(a) Opere como clínica de readaptación o fisioterapia activa;
(b) Cumpla con el pago de la cuota mensual operativa (cláusula CUARTA);
(c) No incurra en alguno de los supuestos de terminación (cláusula DÉCIMA).

La licencia no caduca por paso del tiempo.
...
(e) Soporte técnico prioritario en horario hábil.
(f) Capacitación inicial de su personal y refresher anual.
(g) Influencia formal en el roadmap del producto: hasta 3 features
    priorizadas por año natural, dentro del alcance funcional general.

Sin límites en: número de usuarios (fisios, administrativos, pacientes),
sucursales, citas almacenadas, evaluaciones, notas clínicas, dentro de
parámetros de uso razonable propios del giro de MoveWell.
...
Esta cuota cubre:
- Infraestructura cloud (Vercel, Supabase, monitoreo).
- Mensajería WhatsApp Business API.
- Timbrado CFDI vía Facturama.
- Email transaccional.

Costos extraordinarios pass-through (facturados aparte a costo real,
con desglose):
- Tokens de inteligencia artificial por uso real.
- Storage adicional fuera de cuotas estándar.
...
5.2. MoveWell recibe únicamente licencia de uso, no transmisión de
     ...
     (a) Acceder al código fuente.
     (b) Comercializar, sublicenciar, vender o redistribuir el SaaS.
     (c) Hacer ingeniería inversa.
...
Las partes mantendrán confidencialidad sobre la información comercial,
operativa y técnica intercambiada, durante la vigencia del contrato y
por 5 años después de su terminación.
...
9.2. Tiempos de respuesta:
     - Incidente crítico (plataforma caída o módulo inoperante): 4h hábiles.
     - Incidente menor: 24h hábiles.
     - Solicitud de feature dentro de las 3 priorizadas/año: análisis
       en 30 días.

9.3. Ventana de mantenimiento programado: domingos 2:00-5:00 AM (CST),
     con aviso de 48h.

9.4. En caso de degradación grave del servicio, MoveWell podrá descontar
     pro-rata de la cuota mensual del periodo afectado.
...
Las partes se someten a las leyes mexicanas y a la jurisdicción de los
tribunales de San Luis Potosí, S.L.P., renunciando a cualquier otra que
pudiera corresponderles.

DÉCIMA SEGUNDA — VIGENCIA

Este contrato entra en vigor en la fecha de firma y reemplaza cualquier
acuerdo previo entre las partes, incluyendo la propuesta comercial
KonectAI ↔ MoveWell del 21 de abril de 2026, la cual queda sin efecto
por mutuo acuerdo.
...
- Cláusula de fuerza mayor (caída de Vercel/Supabase no debería penalizar SLA).
...
## 🎤 Cómo presentarlo a MoveWell

**Email/reunión sugerida:**
...
> Esto significa:
>
> - **No pagan los $510,000 MXN** del desarrollo. Cero.
> - **Pagan solo $3,500 MXN/mes** que cubren la infraestructura real (Vercel, WhatsApp, facturación, etc.). Si los costos bajan, pagan menos. Si suben, les avisamos antes.
> - **Acceso de por vida**, gratis, a todo el producto y todas las features futuras.
...
>
> Necesito 30 días para constituir la sociedad y firmar los contratos antes de arrancar a programar. Si están de acuerdo en principio, mando borrador de contrato esta semana.
>
> ¿Te parece si nos vemos el [día] para platicarlo?"

---

## 🚦 Si MoveWell acepta

Vas al **Master Plan + Fase 0 prompt** que ya te pasé, sin cambios. La arquitectura técnica multi-tenant ya está alineada con este modelo.
...
**Próximo paso:** revisá este modelo, decidí si te suena, y agenda la conversación con MoveWell. El tech del Master Plan ya está alineado.
